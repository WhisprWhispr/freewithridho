...
var FALLBACK_SERVER_KEY = process.env.MIDTRANS_SERVER_KEY;
var MIDTRANS_URL = process.env.MIDTRANS_URL || "https://app.sandbox.midtrans.com/snap/v1/transactions";
var SITE_URL = process.env.URL || "http://localhost:5173";
var handler = async (event) => {
  ...
  try {
    ...
    const orderId = `TRX-${Date.now()}-${Math.floor(Math.random() * 9e3 + 1e3)}`;
    const payload = {
      transaction_details: {
        order_id: orderId,
